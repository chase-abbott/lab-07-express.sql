...
app.post('/api/players', async (req, res) => {
  try {
    ...
    const data = await client.query(`
      ...
      RETURNING id, name, position, url_image, year_enrolled as "yearEnrolled", is_transfer as "isTransfer", is_active as "isActive", user_id as "userId";
    `,
    [player.name, player.position, player.url_image, player.yearEnrolled, player.isTransfer, player.isActive, 1]);
...
app.delete('/api/players/:id', async (req, res) => {
  try{
    ...
    DELETE FROM players
    WHERE id = $1
    RETURNING id, name, position, url_image, year_enrolled as "yearEnrolled", is_transfer as "isTransfer", is_active as "isActive";`,
    [req.params.id]);
    res.json(data.rows);
  }
  catch(err) {
